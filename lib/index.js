...
const symPopulateChildren = makeSymbol('populate-children');
...
export class VComponent extends EventEmitter {
  ...
  scanChildren() {
    return this.constructor.render(this.root).then(
      this[symPopulateChildren].bind(this),
    );
  }

  [symPopulateChildren](children) {
    const myChildren = {};
  ...
  listen(type, el, handler, useCapture) {
    ...
    function capture(event) {
      const elements = this.qsa(el);
      const target = event.target;
      if ([].indexOf.call(elements, target) !== -1) {
        handler.call(this, event);
      }
    }

    type.forEach((eventName) => {
      if (typeof el === 'string') { // delegate
